...
int mbr_scan(BlockDevice *dev)
{
    ...
    for (int i = 0; i < MBR_PARTITION_COUNT; i++)
    {
        ...
        if (!mbr_partition_is_used(partition))
            continue;

		if (partition->type == MBRYPE_GPT_PROTECTIVE)
    		return 0; /* done here, let GPT handle that */
        ...
        if (start + count > dev->total_sectors)
            continue;
        ...
        if(blkdev_create_partition(dev, i, start, count))
			created++;
    }
