...
static inline void lzfetch(void)
{
    ...
    kprintf("|	 /|		  (void)		  |\t\t\t\t\t\t\tTheme:	   CalangoGreen\n");
    kprintf("|	/ |		,__\\	 /__,	   |\t\t\t\t\t\t\tCPU:		%s\n", g_cpuid.brand_name);
...
void free(void)
{
    ...
    // u32 free_blocks = pmm_free_block_count(void);
    // u32 used_blocks = pmm_used_block_count(void);
...
void uptime(void)
{
    kprintf("%llu\n", (u64)timespec_uptime(void).sec);
}

void ms(void)
{
    kprintf("%llu\n", (u64)time_uptime_ms(void));
}

static inline void date(void)
{
    TimeSpec time = time_now(void);
