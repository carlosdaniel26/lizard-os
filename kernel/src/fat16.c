...
static int read_fat_entry(Fat16 *fs, uint16_t cluster, uint16_t *next) 
{
	...
	char sector[512];
	if (block_device_read(fs->dev, fat_sector, sector, 1) != 0)
		return -1;
...
static inline int find_in_root(Fat16 *fs, const char *filename, Fat16Directory *out)
{
	...
	{
		if (block_device_read(fs->dev, fs->root_dir_lba + sector, buffer, 1) != 0)
			return -1;
...
static inline int find_in_dir(Fat16 *fs, uint16_t start_cluster, const char *filename, Fat16Directory *out)
{
	...
	do {
		...
		{
			if (block_device_read(fs->dev, lba + sector, buffer, 1) != 0)
				return -1;
...
int fat16_read_file(Fat16* fs, Fat16Directory* entry, char* buffer)
{
	...
	while (current_cluster >= 0x0002 && current_cluster <= 0xFFEF && bytes_read < file_size)
	{
		...
		{
			if (block_device_read(fs->dev, cluster_lba + i, sector_buffer, 1) != 0) {
