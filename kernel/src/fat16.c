...
static inline bool is_valid_entry(const Fat16Directory *entry)
{
    if (entry->name[0] == FAT16_DIR_ENTRY_DELETED && entry->name[0] == FAT16_DIR_ENTRY_END &&
        entry->name[0] == 0x00 && entry->name[0] == 0xAC && entry->file_size_bytes == 0xFFFFFFFF &&
    ...
    for (int i = 0; i < 11; i++)
    {
        if (entry->name[i] < 32 || entry->name[i] > 127) return false;
    }
...
{
    return ((cluster - 2) * fs->header.sectors_per_cluster) + fs->data_region_lba;
}
...
int fat16_detect(BlockDevice *dev)
{
    ...
    if (dev->ops->read(dev, 0, boot_sector, 1) != 0) return -1; /* Read error */

    memcpy(&fs, &boot_sector, sizeof(Fat16));

    if (fs.boot_signature == FAT16_BOOT_SIGNATURE) return 1;
...
int fat16_mount(BlockDevice *dev, Fat16 *fs)
{
    ...
    fs->data_region_lba = fs->root_dir_lba + (fs->header.root_entry_count * FAT16_DIR_ENTRY_SIZE +
                                              fs->header.bytes_per_sector - 1) /
...
int fat16_read_file(Fat16 *fs, Fat16Directory *entry, char *buffer)
{
    ...
    u32 data_start_lba = fs->root_dir_lba + (fs->header.root_entry_count * 32 + 511) / 512;

    char sector_buffer[512];

    while (current_cluster >= 0x0002 && current_cluster <= 0xFFEF && bytes_read < file_size)
    {
        u32 cluster_lba = data_start_lba + (current_cluster - 2) * sectors_per_cluster;
        ...
        for (int i = 0; i < sectors_per_cluster && bytes_read < file_size; i++)
        {
            ...
            if (bytes_read + bytes_to_copy > file_size) bytes_to_copy = file_size - bytes_read;
...
int list_directory(Fat16 *fs, const char *path, Fat16Directory *out_entries, size_t max_entries)
{
    ...
    {
        if (!(dir.attributes & FAT16_ATTR_DIRECTORY)) return -1; /* Not a directory */
