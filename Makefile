...
QEMUHDDFLAGS =  -drive file=fat16.img,format=raw,if=ide
QEMUFLAGS = -no-reboot -d int,cpu_reset -D qemu_log.txt -cdrom $(OUTPUT_ISO) $(QEMUHDDFLAGS) -m 4G -M smm=off -rtc base=localtime -boot order=d
...
run:
	...
	@qemu-system-x86_64 $(QEMUFLAGS) $(KVMFLAGS) 
